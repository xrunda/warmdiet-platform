 ...
 * 运行：node docs/md-to-html-report.js
...
// 简单 Markdown → HTML（段落、标题、粗体、图片、列表、表格、分隔线）
function md2html(text) {
  ...
  while (i < lines.length) {
    ...
    // 行内图片保留
    let para = line;
    para = para.replace(/!\[([^\]]*)\]\(([^)]+)\)/g, '<img src="$2" alt="$1" class="inline-img">');
    html += `<p>${bold(esc(para))}</p>\n`;
  }
...
<head>
  ...
  <style>
    * { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    .print-hint a { color: #fff; text-decoration: underline; }
  </style>
</head>
<body>
  <p class="print-hint no-print">打印本页为 PDF：<kbd>Cmd+P</kbd>（Mac）或 <kbd>Ctrl+P</kbd>（Win）→ 目标选择「另存为 PDF」</p>
  <div class="cover">
    <h1>帮奶奶“管住嘴、记性好”</h1>
...
console.log('请用浏览器打开该文件，按 Cmd+P（Mac）或 Ctrl+P（Win），选择「另存为 PDF」即可得到精美 PDF。');
