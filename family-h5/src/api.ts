...
const LOCAL_MEALS_KEY = 'family_demo_meals';
...
function getLocalMealCache(): Record<string, any[]> {
  try {
    return JSON.parse(localStorage.getItem(LOCAL_MEALS_KEY) || '{}');
  } catch {
...
function getLocalMeals(patientId: string): any[] {
  return getLocalMealCache()[patientId] || [];
}

function cacheLocalMeal(patientId: string, meal: any): void {
  const cache = getLocalMealCache();
  const current = cache[patientId] || [];
  cache[patientId] = [meal, ...current.filter((item) => item.id !== meal.id)].slice(0, 50);
  localStorage.setItem(LOCAL_MEALS_KEY, JSON.stringify(cache));
}

function mergeMeals(remoteMeals: any[] = [], localMeals: any[] = []): any[] {
  const byId = new Map<string, any>();
  [...localMeals, ...remoteMeals].forEach((meal) => {
    if (meal?.id) byId.set(String(meal.id), meal);
  });
  return Array.from(byId.values()).sort((a, b) => {
    const aTime = `${a.mealDate || a.meal_date || ''} ${a.mealTime || a.meal_time || ''}`;
    const bTime = `${b.mealDate || b.meal_date || ''} ${b.mealTime || b.meal_time || ''}`;
...
export async function fetchDashboard() {
  ...
  const data = await request<any>(`/patients/${patientId}/dashboard`);
  const meals = mergeMeals(data.meals || data.latestMeals || [], getLocalMeals(patientId));
  return { ...data, meals, latestMeals: meals.slice(0, 6) };
}
...
export async function fetchPatientMeals(date?: string) {
  ...
  const remoteMeals = await request<any[]>(`/meals/patient/${patientId}${query}`);
  const localMeals = getLocalMeals(patientId).filter((meal) => !date || meal.mealDate === date || meal.meal_date === date);
  return mergeMeals(remoteMeals, localMeals);
}
...
export async function createMeal(data: {
  ...
  const patientId = getCurrentPatientId();
  if (!patientId) throw new Error('请先登录');
  const meal = await request<any>(`/meals/patient/${patientId}`, { method: 'POST', body: data });
  cacheLocalMeal(patientId, meal);
  return meal;
}
