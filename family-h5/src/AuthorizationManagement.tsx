...
function normalizeAuthorization(raw: any): DoctorAuthorization {
  const dataRange = (raw?.scope?.dataRange || raw?.scopeDataRange || raw?.dataRange || 'recent_30d') as DataRange;
  return {
    ...raw,
    doctorName: raw?.doctorName || '测试医生',
    hospital: raw?.hospital || raw?.hospitalName || '三餐管家合作医院',
    department: raw?.department || '营养科',
    licenseNumber: raw?.licenseNumber || 'demo-license',
    authorizationType: Array.isArray(raw?.authorizationType) ? raw.authorizationType : ['meal_records'],
    authorizedAt: raw?.authorizedAt || raw?.createdAt || new Date().toISOString(),
    status: raw?.status || 'active',
    scope: {
      startDate: raw?.scope?.startDate || raw?.scopeDataStart || raw?.authorizedAt?.slice(0, 10) || new Date().toISOString().slice(0, 10),
      endDate: raw?.scope?.endDate || raw?.scopeDataEnd,
      dataRange,
    },
...
const AuthorizationCard = ({
  ...
  const { authorizationType, scope, expiresAt, lastAccessedAt, accessCount } = normalized;
    ...
    >
      <div className="flex items-start justify-between mb-3">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h4 className="text-base font-bold text-text-main">{normalized.doctorName}</h4>
            <div className="flex items-center gap-1 text-sm text-text-sub mt-0.5">
              <Hospital className="w-3.5 h-3.5" />
              <span>{normalized.hospital} · {normalized.department}</span>
            </div>
            <div className="text-xs text-text-hint mt-1">执业证：{normalized.licenseNumber}</div>
          </div>
        </div>
        <div className={cn(
      ...
      <div className="space-y-2 mb-3">
        ...
        <div className="flex items-center gap-1 text-sm text-text-sub">
          ...
          <span>数据范围：{DATA_RANGE_LABELS[scope.dataRange] || DATA_RANGE_LABELS.recent_30d}</span>
        </div>
        <div className="flex items-center gap-1 text-sm text-text-sub">
...
const AuthorizationDetailModal = ({
  ...
  const normalized = normalizeAuthorization(authorization);
  const { authorizationType, expiresAt, lastAccessedAt, accessCount } = normalized;
        ...
        >
          <div className="flex items-start justify-between mb-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h4 className="text-lg font-bold text-text-main">{normalized.doctorName}</h4>
                <div className="text-sm text-text-sub">
                  {normalized.hospital} · {normalized.department}
                </div>
              </div>
            </div>
              ...
              <div className="space-y-2 text-sm">
                <div className="flex justify-between">
                  ...
                  <span className="text-text-main">
                    {new Date(normalized.authorizedAt).toLocaleString('zh-CN')}
                  </span>
                </div>
                <div className="flex justify-between">
...
export const AuthorizationManagement = () => {
  ...
  useEffect(() => {
    fetchPatientAuthorizationsDetailed()
      .then((data: any) => setAuthorizations((Array.isArray(data) ? data : []).map(normalizeAuthorization)))
    ...
    createAuthorization({
      ...
      .then((created: any) => {
        ...
        return fetchPatientAuthorizationsDetailed().then((list: any) => {
          setAuthorizations((Array.isArray(list) ? list : []).map(normalizeAuthorization));
          return created;
        });
      })
      .catch((err) => {
