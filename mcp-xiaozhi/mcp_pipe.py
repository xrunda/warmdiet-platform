...
async def pipe_websocket_to_process(websocket, process, target):
    ...
    try:
        while True:
            ...
            # Track MCP request types from xiaozhi side
            ...
            try:
                ...
                if method:
                    logger.info(f"[{target}] [WS->STDIO] method={method} id={req_id}")
            except Exception:
                pass
...
async def pipe_process_to_websocket(process, websocket, target):
    ...
    try:
        while True:
            ...
            # Track MCP responses/events from local stdio server
            try:
                ...
                if method:
                    logger.info(f"[{target}] [STDIO->WS] method={method} id={req_id}")
                elif req_id is not None:
                    logger.info(f"[{target}] [STDIO->WS] response id={req_id} error={has_error}")
            except Exception:
                pass
